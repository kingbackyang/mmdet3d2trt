...
def hard_voxelize(points, voxels, coors, num_points_per_voxel, voxel_size, coors_range, max_points, max_voxels):
    N = points.shape[0]
    num_features = points.shape[1]
    voxel_num = 0
    ndim = points.shape[1] - 1
    ndim_minus_1 = ndim - 1
    grid_size = [int(round(i)) for i in
                 list(np.around((np.array(coors_range[ndim:]) - np.array(coors_range[:ndim])) / np.array(voxel_size)))]
    voxelmap_shape = tuple(np.round(grid_size).astype(np.int32).tolist())
    voxelmap_shape = voxelmap_shape[::-1]
    coor_to_voxelidx = np.full(voxelmap_shape, -1, dtype=np.int32)
    coor = [0, 0, 0]
    for i in range(N):
        failed = False
        for j in range(ndim):
            c = int((points[i, j] - coors_range[j]) / voxel_size[j])
        ...
        if num < max_points:  # origin process
            for m in range(num_features):
                voxels[voxelidx, num, m] = points[i, m]
            num_points_per_voxel[voxelidx] += 1
    voxel_mask = torch.zeros(max_voxels, dtype=torch.float32)
    voxel_mask[:voxel_num] = 1
    return voxel_mask, voxels, coors, num_points_per_voxel
...
class Voxelization(nn.Module):
    ...
    def forward(self, points):
        ...
        voxel_num, voxels_proc, coors_proc, num_points_per_voxel_proc = hard_voxelize(points, voxels, coors, num_points_per_voxel, self.voxel_size, self.coors_range, self.max_points, self.max_voxels)
        return voxel_num, voxels_proc, coors_proc, num_points_per_voxel_proc
        # # select the valid voxels
        # voxels_out = voxels_proc[:voxel_num]
        # coors_out = coors_proc[:voxel_num]
        # num_points_per_voxel_out = num_points_per_voxel_proc[:voxel_num]
...
class PillarFeature(nn.Module):
    ...
    def __init__(self,
                 ...
                 norm_cfg=dict(type='BN1d', eps=1e-3, momentum=0.01),
                 mode='max'):
        super(PillarFeature, self).__init__()
        self.type = type
        assert len(feat_channels) > 0
        if with_cluster_center:
            in_channels += 3
    ...
    # def forward(self, voxels, voxel_mask, mask, coors_rw, num_points_rw):
    #     """Forward function.
    #
    #     Args:
    #         features (torch.Tensor): Point features or raw points in shape
    ...
    #         torch.Tensor: Features of pillars.
    #     """
    #     voxel_num = int(torch.sum(voxel_mask))
    #     features = voxels[:voxel_num]
    #     coors = coors_rw[:voxel_num]
    #     num_points = num_points_rw[:voxel_num]
    #     mask = mask[:voxel_num]
    #
    #     features_ls = [features]
    #     # Find distance of x, y, and z from cluster center
...
class VoxelNetPfn(nn.Module):

    def __init__(self, voxel_encoder):
        super(VoxelNetPfn, self).__init__()
        self.voxel_encoder = PillarFeature(**voxel_encoder)
    ...
    def forward(self, pillar_x, pillar_y, pillar_z, pillar_i, num_voxels, x_sub_shaped, y_sub_shaped, mask):
        ...
        """
        outs = self.voxel_encoder(pillar_x, pillar_y, pillar_z, pillar_i, num_voxels, x_sub_shaped, y_sub_shaped, mask)

        return outs


class VoxelNetRpn(nn.Module):
    r"""`VoxelNet <https://arxiv.org/abs/1711.06396>`_ for 3D detection."""

    def __init__(self, backbone, neck, bbox_head):
        super(VoxelNetRpn, self).__init__()
...
if __name__ == "__main__":
    ...
    slimming_json = "/home/mk/mmdetection3d-master/benchmark_model/pp_slim/slim_description/depth_description_0.5.json"
    ...
    model_mmdet = torch.load("/home/mk/mmdetection3d-master/benchmark_model/pp_slim/slim_0.5/epoch_150.pth")
    # voxel_encoder_dict = model_mmdet["state_dict"]
    # del_list = []
    # for key in voxel_encoder_dict.keys():
    #     if key.startswith("voxel_encoder"):
    #         pass
    #     else:
    #         del_list.append(key)
    # for var in del_list:
    #     del voxel_encoder_dict[var]
    #
    # voxelize_model = Voxelization(**network_arch["voxel_layer"])
    # model_pfn = VoxelNetPfn(network_arch["voxel_encoder"]).cuda()
    # vx = model_pfn.voxel_encoder.vx
    # vy = model_pfn.voxel_encoder.vy
    #
    # # mask = get_paddings_indicator(pfn_input_num_points, pfn_input_voxels.shape[1], axis=0)
    # # mask = torch.unsqueeze(mask, -1).type_as(pfn_input_voxels)
    # model_pfn.load_state_dict(voxel_encoder_dict)
    # model_pfn.eval()
    # points = torch.randn(19364, 4)
    # voxel_mask, voxels, coors, num_points = voxelize_model(points)
    # coor_list = [coors]
    # coors_batch = []
    ...
    # x_offset = model_pfn.voxel_encoder.x_offset
    # y_offset = model_pfn.voxel_encoder.y_offset
    # point_cloud_range = model_pfn.voxel_encoder.point_cloud_range
    ...
    #
    #
    # pillar_x = torch.ones(40000, 32, 1).cuda()
    # pillar_y = torch.ones(40000, 32, 1).cuda()
    # pillar_z = torch.ones(40000, 32, 1).cuda()
    # pillar_i = torch.ones(40000, 32, 1).cuda()
    # x_sub_shaped = torch.ones(40000, 32, 1).cuda()
    # y_sub_shaped = torch.ones(40000, 32, 1).cuda()
    # pfn_input_num_points = torch.ones(40000).cuda()
    # pfn_input_mask = torch.ones(40000, 32, 1).cuda()
    # torch.onnx.export(model_pfn, (pillar_x, pillar_y, pillar_z, pillar_i, pfn_input_num_points, x_sub_shaped, y_sub_shaped, pfn_input_mask), "pfn_slim.onnx", verbose=True, opset_version=11)
    # out = model_pfn(pillar_x, pillar_y, pillar_z, pillar_i, pfn_input_num_points, x_sub_shaped, y_sub_shaped, pfn_input_mask)
    # print(out)


    #
    rpn_model_dict = model_mmdet["state_dict"]
    del_list = []
    for key in rpn_model_dict.keys():
        if key.startswith("voxel_encoder"):
            del_list.append(key)
        else:
            pass
    ...
    for var in del_list:
        del rpn_model_dict[var]
    model_rpn = VoxelNetRpn(network_arch["backbone"], network_arch["neck"], network_arch["bbox_head"]).cuda()
    model_rpn.load_state_dict(model_mmdet["state_dict"])
    rpn_input = torch.ones((1, 64, 496, 432)).cuda()
    # data_input = [torch.randn(19364, 4).cuda()]
    # dynamic_axes = {"points": [0]}
    #
    model_rpn.eval()
    model_rpn = model_rpn.cuda()
    torch.onnx.export(model_rpn, rpn_input, "rpn_slim.onnx", verbose=True, opset_version=11)
    # out = model_rpn(rpn_input)
    # print(out)
